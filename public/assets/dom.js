...
$(document).ready(() => {
  ...
  $('#map').load('./assets/map.svg', () => {
    ...
    try {
      if (!storageAvailable('localStorage')) { throw 'No localStorage available'}
    } catch(e) {
      alert(
        "Sorry, local storage isn't available in your browser. That means we can't save the data you upload."
      );
    } finally {
      if (localStorage.getItem('stations') === null && !isMobile) {
        $('#welcome').fadeIn(500);
...
$('#welcomeStnInput').on('keyup', (e) => {
  if (e.key === 'Enter' || e.keyCode === 13) {
    $('#welcome').fadeOut(500);
    usrData('save', 'stations', $('#welcomeStnInput').val());
    addStnsToMap($('#welcomeStnInput').val());
    updateLineSegs($('#welcomeStnInput').val());
  }
})
...
$('#stationInput').on('keyup', (e) => {
  if (e.key === 'Enter' || e.keyCode === 13) {
    addStnsToMap($('#stationInput').val());
    usrData('save', 'stations', $('#stationInput').val());
    updateLineSegs($('#welcomeStnInput').val());
  }
});